  ...
  const ControlsContent = () => (
    <>
      ...
      <div className="flex justify-between items-baseline mb-0.5">
        <span className="text-[9px] font-medium tracking-[0.05em] uppercase text-white/65">Scale</span>
        <span className="text-[10px] tabular-nums text-white/45 font-nhg">{params.scale}</span>
      </div>
      ...
      {/* Particles */}
      <div className="flex justify-between items-baseline mb-0.5">
        <span className="text-[9px] font-medium tracking-[0.05em] uppercase text-white/65">Particles</span>
        <span className="text-[10px] tabular-nums text-white/45 font-nhg">{params.count}</span>
      </div>
      ...
      <div className="flex justify-between items-baseline mb-0.5">
        ...
        <span className="text-[10px] tabular-nums text-white/45 font-nhg">{params.speed.toFixed(1)}</span>
      </div>
      ...
      {/* Fade */}
      <div className="flex justify-between items-baseline mb-0.5">
        <span className="text-[9px] font-medium tracking-[0.05em] uppercase text-white/65">Trail Fade</span>
        <span className="text-[10px] tabular-nums text-white/45 font-nhg">{params.fade}</span>
      </div>
      ...
      <div className="flex flex-wrap gap-1 mb-3">
        {Object.entries(PALETTES).map(([key, val]) => (
          <button key={key}
            className={`px-2.5 py-1 text-[8px] font-medium tracking-[0.05em] uppercase border rounded-sm transition-all duration-200 cursor-pointer
              ${currentPalette === key
      ...
      {/* Stats */}
      <div className="mt-3 p-2 rounded-sm font-nhg text-[9px] leading-relaxed text-white/30"
      ...
      <div className="flex flex-wrap gap-2 mt-2.5">
        {[['H','hide'],['R','reset'],['S','save'],['␣','pause']].map(([k, label]) => (
          <span key={k} className="text-[9px] text-white/30 flex items-center gap-1">
            <code className="px-1 py-px border border-white/10 rounded-sm font-nhg text-[9px] text-white/35">{k}</code>
            {label}
          </span>
        ))}
      </div>
